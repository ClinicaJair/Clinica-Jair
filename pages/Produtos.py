...
def gerar_relatorio_pdf(df):
    ...
    pdf.cell(0, 8, limpar_texto("SISTEMA DE GESTÃO DE PRODUTOS"), ln=True, align="C")
    pdf.set_font("helvetica", "", 10)
    pdf.cell(
        0,
        6,
        limpar_texto("Relatório Geral de Cadastros"),
        ln=True,
        align="C",
    )
    ...
    for _, row in df.iterrows():
        codigo = limpar_texto(row.get("codigo", "")) if pd.notna(row.get("codigo")) else ""
        nome = limpar_texto(row.get("nome", "")) if pd.notna(row.get("nome")) else ""
        telefone = limpar_texto(row.get("telefone", "")) if pd.notna(row.get("telefone")) else ""

        if len(nome) > 45:
            nome = nome[:42] + "..."

        pdf.cell(largura_codigo, 7, f" {codigo}", border=1)
        pdf.cell(largura_nome, 7, f" {nome}", border=1)
        pdf.cell(largura_tel, 7, f" {telefone}", border=1, ln=True)
...
def dialog_confirmar_deletar(codigo_deletar):
    ...
    with col_sim:
        if st.button("Sim", use_container_width=True, type="primary"):
            try:
                supabase.table("produtos").delete().eq("codigo", codigo_deletar).execute()
...
produto_sel = st.session_state.produto_selecionado
id_ativo = produto_sel.get("codigo") if produto_sel else None
...
with col_esquerda:
    if id_ativo:
        st.markdown(f"##### ✏️ Editando Produto: Código {id_ativo}")
    ...
    with st.form(key=f"form_cliente_{st.session_state.update_trigger}"):
        ...
        with col2:
            nome = st.text_input(
                "Nome", value=produto_sel.get("nome", "") if produto_sel else ""
            )
        with col3:
            data_inicial = None
            if produto_sel and produto_sel.get("data_nascimento"):
                try:
                    data_inicial = pd.to_datetime(
                        produto_sel["data_nascimento"], dayfirst=True
                    ).date()
                except Exception:
                    data_inicial = None
        ...
        with col4:
            cpf = st.text_input(
                "CPF", value=produto_sel.get("cpf", "") if produto_sel else ""
            )
        with col5:
            rg = st.text_input(
                "RG", value=produto_sel.get("rg", "") if produto_sel else ""
            )
        with col6:
            crm = st.text_input(
                "CRM", value=produto_sel.get("crm", "") if produto_sel else ""
            )
        ...
        with col7:
            cep = st.text_input(
                "CEP", value=produto_sel.get("cep", "") if produto_sel else ""
            )
        with col8:
            endereco = st.text_input(
                "Endereço", value=produto_sel.get("endereco", "") if produto_sel else ""
            )
        with col9:
            estado = st.text_input(
                "Estado", value=produto_sel.get("estado", "") if produto_sel else ""
            )
        ...
        with col10:
            bairro = st.text_input(
                "Bairro", value=produto_sel.get("bairro", "") if produto_sel else ""
            )
        with col11:
            cidade = st.text_input(
                "Cidade", value=produto_sel.get("cidade", "") if produto_sel else ""
            )
        ...
        with col12:
            telefone = st.text_input(
                "Telefone", value=produto_sel.get("telefone", "") if produto_sel else ""
            )
        with col13:
            telefone1 = st.text_input(
                "Telefone 2",
                value=produto_sel.get("telefone1", "") if produto_sel else "",
            )
        with col14:
            email = st.text_input(
                "E-mail", value=produto_sel.get("email", "") if produto_sel else ""
            )
        ...
        with col14_2:
            instagram = st.text_input(
                "Instagram", value=produto_sel.get("instagram", "") if produto_sel else ""
            )
...
with col_direita:
    ...
    if not df_dados.empty:
        ...
        evento_selecao = st.dataframe(
            ...
            key=f"tabela_produtos_{st.session_state.table_key}",
            height=420,
        )
        ...
        if linhas_selecionadas:
            ...
            if st.session_state.produto_selecionado != dados_linha:
                st.session_state.produto_selecionado = dados_linha
                st.rerun()
    else:
        st.info("Nenhum registro cadastrado ou encontrado.")
...
if submit_criar:
    if nome:
        try:
            ...
            st.session_state["mensagem_sucesso"] = f"Registro cadastrado com sucesso sob o Código {codigo}!"
            limpar_campos_e_selecao()
            st.rerun()
        except Exception as e:
...
if submit_atualizar:
    if id_ativo:
        try:
            supabase.table("produtos").update(payload).eq("codigo", id_ativo).execute()
